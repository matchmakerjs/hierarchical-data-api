...
export class IndexController {
  ...
  index(
    ...
    return new Promise<void>((res, rej) => {
      try {
        const swaggerUIPath =
          process.env.SWAGGER_UI_PATH || "./swagger-ui.html";
        fs.stat(swaggerUIPath, (err) => {
          ...
          if (err.code === "ENOENT") {
            return rej(
              new ErrorResponse(404, { message: "Swagger UI not available" })
            );
          }
          rej(
            new ErrorResponse(500, { message: "Unable to fetch Swagger UI" })
          );
        });
      } catch (error) {
        rej(error);
    ...
    return new Promise<void>((res, rej) => {
      try {
        ...
        fs.stat(docsPath, (err) => {
          ...
          if (err.code === "ENOENT") {
            return rej(
              new ErrorResponse(404, { message: "API docs not available" })
            );
          }
