...
describe('item relationship', () => {
    ...
    it('should order nested items', async () => {
        ...
        const ancestors = await container.getInstance(ItemRelationshipService).getAncestors([list.pop()]);
    ...
    it('should update tree', async () => {
        ...
        const a1 = await itemService.createItem({ name: randomUUID(), parentId: a.id });

        const response = await TestServer(container, {} as JwtClaims).put(`/items/${a.id}`, {
            name: a.name,
            parentId: b.id,
        } as ItemApiRequest);
        expect(response.statusCode).toBe(200);
        const list = await container.getInstance(EntityManager).find(ItemRelationship, {
            where: {
                descendant: {
                    id: In([a.id, a1.id]),
                },
        ...
        expect(list.filter((it) => it.descendant.toString() === a1.id).map((it) => it.ancestor)).toEqual([a.id, b.id]);
        expect(list.filter((it) => it.descendant.toString() === a.id).map((it) => it.ancestor)).toEqual([b.id]);
    });
    ...
    it('should update relationships on update', async () => {
        ...
        for (let i = 0; i < 7; i++) {
            ...
            entities.push(await container.getInstance(ItemService).createItem(requests.at(-1)));
        }
        ...
        for (let i = start + 1; i < requests.length; i++) {
            ...
            const response = await TestServer(container, {} as JwtClaims).put(`/items/${entry.id}`, {
                name: entry.name,
        ...
        for (let i = 0; i < start; i++) {
            const entry = entities[i];
            const response = await TestServer(container, {} as JwtClaims).put(`/items/${startItem.id}`, {
                name: startItem.name,
                parentId: entry.id,
            } as ItemApiRequest);
            expect(response.statusCode).toBe(200);
            expect((response.parseJson() as ItemApiResponse).ancestors.length).toBe(1);
        }
        const ancestors = await container.getInstance(ItemRelationshipService).getAncestors([last.id]);
    ...
    it('should prevent cyclic dependency', async () => {
        ...
        const last = entities.at(-1);
        const response2 = await TestServer(container, {} as JwtClaims).put(`/items/${first.id}`, {
            name: first.name,
            parentId: last.id,
        } as ItemApiRequest);
    ...
    it('should delete parent and children', async () => {
        ...
        const response = await TestServer(container, {} as JwtClaims).delete(`/items/${parent.id}`);
