...
describe('item search', () => {
    ...
    beforeEach(async () => {
        await container.getInstance(EntityManager).createQueryBuilder(ItemRelationship, 'e').delete().execute();
        await container.getInstance(EntityManager).createQueryBuilder(Item, 'e').delete().execute();
    });
    ...
    it('should search', async () => {
        ...
        await Promise.all(requests.map((it) => container.getInstance(ItemService).createItem(it)));
        const response = await TestServer(container).get(`/items`);
        expect(response.statusCode).toBe(200);
        const searchResult: SearchResult<ItemApiResponse> = response.parseJson();
        expect(searchResult).toBeDefined();
        expect(searchResult.total).toBe(requests.length);
        expect(searchResult.results.map((it) => it.numberOfChildren).sort()).toEqual([0, 0]);
    });
    ...
    it('should search by parent', async () => {
        ...
        const parent = await container.getInstance(ItemService).createItem(requests[0]);
        requests[1].parentId = parent.id;
        await container.getInstance(ItemService).createItem(requests[1]);
        const response = await TestServer(container).get(`/items?parent=${parent.id}`);
        expect(response.statusCode).toBe(200);
        const searchResult: SearchResult<ItemApiResponse> = response.parseJson();
        expect(searchResult).toBeDefined();
    ...
    it('should return count of children', async () => {
        ...
        expect(searchResult).toBeDefined();
        expect(searchResult.total).toBe(requests.length);
        expect(searchResult.results.map((it) => it.numberOfChildren).sort()).toEqual([0, 1]);
    });
});
